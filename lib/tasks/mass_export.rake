    ...
    dumps.each_key do |dump|
      ...
      works.each {|m|
        begin
          ...
          pseudocatalogue << [m.id, ppath+fname, m.title, m.expressions[0].works[0].authors.map{|x| x.name}.join('; '), m.expressions[0].translation ? m.expressions[0].translators.map{|x| x.name}.join('; ') : '', m.expressions[0].works[0].authors.map{|x| x.wikidata_qid.to_s}.join('; '), m.expressions[0].works[0].translators.map{|x| x.wikidata_qid.to_s}.join('; '), m.expressions[0].translation ? m.expressions[0].works[0].orig_lang : '', I18n.t(m.expressions[0].works[0].genre), m.expressions[0].source_edition || '']
          tot[:works] += 1
