...
task :strip_nikkud => :environment do
  basedir =  Rails.configuration.constants['base_dir'] # environment-sensitive constant
  tot = { :files => 0, :stripped => 0, :errors => 0 }
  the_insert = File.open(Rails.configuration.constants['no_nikkud_insert'], 'r:UTF-8').read
