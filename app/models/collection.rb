class Collection < ApplicationRecord
  ...
  def editors_string
    auths = self.involved_authorities.where(role: 'editor')
    if auths.count > 0
      return auths.map(&:authority).map(&:name).join(', ')
    else
      parent_collections.each{|pc| # iterate until we find editors
        s = pc.editors_string
        return s if s.present?
      }
    end
  ...
  def append_collection_item(item)
    item.seqno = self.collection_items.maximum(:seqno).to_i + 1
    item.save!
  end
