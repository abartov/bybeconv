...
module Lexicon
  ...
  class ParseCitation < ApplicationService
    ...
    def call(list_item, subject)
      citation = LexCitation.new(status: :raw, raw: list_item.inner_html, subject: subject)
      ...
      if match
        ...
        rest_text = rest_text.gsub(match[1], '').strip # Remove the matched pages pattern
      end
