...
#    headers['Access-Control-Allow-Origin'] = 'http://benyehuda.org/'
  ...
  def prep_edit_toc
    ...
    @fresh_works = @works.select{|m| @toc.index('מ'+m.id.to_s).nil? }.sort_by{|m| m.id} # in the fresh_works context, the order of creation is more useful, and created_at creates a jumble because its granularity is too coarse
